...
public class Issue2TelephoneBook {

    private HashMap<String, List<String>> book;

    public Issue2TelephoneBook() {
    ...
    public void add(String surname, String number) {
        if (book.containsKey(surname)) {
            ...
            if (!numbers.contains(number)) {
                numbers.add(number);
                System.out.println(String.format("Номер %s записан для фамилии %s", number, surname));
            } else {
    ...
    public List<String> get(String surname) {
        if (book.containsKey(surname)) {
            ...
            System.out.println(String.format("В телефонном справочнике нет данных для фамилии %s", surname));
